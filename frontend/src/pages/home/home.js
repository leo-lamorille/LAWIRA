...
export default function Home() {
  ...
  const [lightPic, setLightPic] = useState(
      "../../../framePics/LIGHT_OFF_PIC.png");
  const [lightHalo, setLightHalo] = useState({})
  const lightRef = useRef(null);

  // <!-- src="../../../framePics/LIGHT_OFF_PIC.png" -->
  function scrollEvent(e) {
    const lightDist = lightRef.current.offsetTop;
    const windowDist = window.pageYOffset;
    if(windowDist >= lightDist - 200){
      setLight(0);
  ...
  return (
      <div>
        ...
        <div className="second-part">
          ...
          <div className="lightFrame" ref={lightRef}>
            <a href="" onClick={Light}><img className="lightPic"
                                            src={lightPic}/></a>
          </div>
